...
async def serve_gmail(request):
    ...
    if match_object is None:
        cozmo.logger.warning("Could not parse sender email address: " + from_email_address)
        email_local_part = from_email_address
    else:
    ...
    async def read_name():
        try:
            async with robot.perform_off_charger():
                ...
                await robot.play_anim_trigger(cozmo.anim.Triggers.ReactToPokeStartled).wait_for_completed()  # pyright: ignore[reportAttributeAccessIssue]
