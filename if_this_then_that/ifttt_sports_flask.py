...
def then_that_action(alert_body):
    ...
    try:
        with robot.perform_off_charger():  # pyright: ignore[reportOptionalMemberAccess]
            '''If necessary, Move Cozmo's Head and Lift to make it easy to see Cozmo's face.'''
            robot.get_in_position()  # pyright: ignore[reportOptionalMemberAccess]

            # First, have Cozmo play an animation
            robot.play_anim_trigger(cozmo.anim.Triggers.ReactToPokeStartled).wait_for_completed()  # pyright: ignore[reportOptionalMemberAccess, reportAttributeAccessIssue]

            # Next, have Cozmo speak the text from the in-game update.
            robot.say_text(alert_body).wait_for_completed()  # pyright: ignore[reportOptionalMemberAccess]

            # Last, have Cozmo display a sports image on his face.
            robot.display_image_file_on_face("../face_images/ifttt_sports.png")  # pyright: ignore[reportOptionalMemberAccess]
