...
def cozmo_program(robot: cozmo.robot.Robot):
    ...
    max_dst, targ = 0, None
    for cube in cubes:  # pyright: ignore[reportGeneralTypeIssues]
        translation = robot.pose - cube.pose
        dst = translation.position.x ** 2 + translation.position.y ** 2
        if dst > max_dst:
            max_dst, targ = dst, cube

    if len(cubes) < 3:  # pyright: ignore[reportArgumentType]
        print("Error: need 3 Cubes but only found", len(cubes), "Cube(s)")  # pyright: ignore[reportArgumentType]
    else:
        robot.pickup_object(targ, num_retries=3).wait_for_completed()  # pyright: ignore[reportUnusedCoroutine]
