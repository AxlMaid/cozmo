...
def cozmo_program(robot: cozmo.robot.Robot):
    ...
    if len(cubes) < 2:  # pyright: ignore[reportArgumentType]
        print("Error: need 2 Cubes but only found", len(cubes), "Cube(s)")  # pyright: ignore[reportArgumentType]
    else:
        # Try and pickup the 1st cube
        current_action = robot.pickup_object(cubes[0], num_retries=3)  # pyright: ignore[reportIndexIssue]
        current_action.wait_for_completed()  # pyright: ignore[reportUnusedCoroutine]
        ...
        current_action = robot.place_on_object(cubes[1], num_retries=3)  # pyright: ignore[reportIndexIssue]
        current_action.wait_for_completed()  # pyright: ignore[reportUnusedCoroutine]
        if current_action.has_failed:
            code, reason = current_action.failure_reason
            result = current_action.result
