...
class BlinkyCube(cozmo.objects.LightCube):
    ...
    def start_light_chaser(self):
        ...
        self._chaser = asyncio.ensure_future(_chaser())
...
def cozmo_program(robot: cozmo.robot.Robot):
    ...
    cube.start_light_chaser()  # pyright: ignore[reportAttributeAccessIssue]
    try:
        print("Waiting for cube to be tapped")
        cube.wait_for_tap(timeout=10)  # pyright: ignore[reportAttributeAccessIssue]
        print("Cube tapped")
    except asyncio.TimeoutError:
        print("No-one tapped our cube :-(")
    finally:
        cube.stop_light_chaser()  # pyright: ignore[reportAttributeAccessIssue]
        cube.set_lights_off()  # pyright: ignore[reportAttributeAccessIssue]
