...
def get_in_position(robot: cozmo.robot.Robot):
    ...
    if (robot.lift_height.distance_mm > 45) or (robot.head_angle.degrees < 40):  # pyright: ignore[reportOptionalOperand]
        with robot.perform_off_charger():  # pyright: ignore[reportGeneralTypeIssues]
            ...
            head_action = robot.set_head_angle(cozmo.robot.MAX_HEAD_ANGLE,
                                               in_parallel=True)
            lift_action.wait_for_completed()  # pyright: ignore[reportUnusedCoroutine]
            head_action.wait_for_completed()  # pyright: ignore[reportUnusedCoroutine]


def calc_pixel_threshold(image: Image.Image):
    ...
    mean_value = np.mean(grayscale_image.get_flattened_data())
...
def cozmo_face_mirror(robot: cozmo.robot.Robot):
    ...
    while True:
        ...
        if latest_image is not None:
            ...
            resized_image = resized_image.transpose(Image.Transpose.FLIP_LEFT_RIGHT)
