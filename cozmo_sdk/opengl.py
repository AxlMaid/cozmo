...
_resource_package = 'cozmo'  # All resources are in the cozmo package's assets/ subdirectory
...
def LoadMtlFile(filename):
    ...
    resource_path = '/'.join(('assets', filename))  # Note: Deliberately not os.path.join, for use with importlib.resources
    file_data = importlib.resources.files(_resource_package).joinpath(resource_path).open('rb')
    ...
    for line in file_data:
        ...
        if attribute_name == 'newmtl':
            ...
            image_name = values[1]
            image_resource_path = '/'.join(('assets', image_name))  # Note: Deliberately not os.path.join, for use with importlib.resources
            image_file_data = importlib.resources.files(_resource_package).joinpath(image_resource_path).open('rb')
