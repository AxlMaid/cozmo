...
class QuickTapGame:
    ...
    def __init__(self, robot: cozmo.robot.Robot):
        ...
        self.cubes: Optional[List[Optional[BlinkyCube]]] = None
        self.countdown_cube: Optional[BlinkyCube] = None

        self.buzzer_display_type: Optional[str] = None

        self.round_start_time = time.time()
        self.quick_tap_player_1: Optional[QuickTapPlayer] = None
        self.quick_tap_player_2: Optional[QuickTapPlayer] = None
    ...
    async def game_round(self):
        ...
        await self.countdown_cube.countdown()  # pyright: ignore[reportOptionalMemberAccess]
        ...
        if obj.object_id is not None:
            if self.quick_tap_state == CHOOSE_CUBES_STATE:
                if self.cozmo_player.cube is None:
                    ...
                    self.player.cube.set_lights_off()  # pyright: ignore[reportOptionalMemberAccess]
            elif self.quick_tap_state == GAME_STATE:
                self.turn_off_buzzer_cubes()
                if obj.object_id == self.player.cube.object_id:  # pyright: ignore[reportOptionalMemberAccess]
                    self.player.register_tap(self.round_start_time)
                elif obj.object_id == self.cozmo_player.cube.object_id:  # pyright: ignore[reportOptionalMemberAccess]
    ...
    async def determine_result_of_round(self):
        ...
        if self.quick_tap_player_1:
            if self.buzzer_display_type == MAKE_BUZZERS_SAME_COLORS:
                ...
                await self.quick_tap_player_1.cube.flair_correct_tap()  # pyright: ignore[reportOptionalMemberAccess]
            elif self.buzzer_display_type == MAKE_BUZZERS_DIFFERENT_COLORS or self.buzzer_display_type == MAKE_BUZZERS_RED:
                self.quick_tap_player_2.wins_round()  # pyright: ignore[reportOptionalMemberAccess]
                await self.quick_tap_player_1.cube.flair_incorrect_tap()  # pyright: ignore[reportOptionalMemberAccess]
            self.report_scores()
    ...
    def determine_first_tapper(self):
        ...
        if self.player.has_tapped or self.cozmo_player.has_tapped:
            if self.cozmo_player.elapsed_tap_time < self.player.elapsed_tap_time:  # pyright: ignore[reportOperatorIssue]
    ...
    async def cozmo_anim_reaction(self):
        ...
        if self.cozmo_player.won_round:
            await self.robot.play_anim_trigger(cozmo.anim.Triggers.OnSpeedtapHandCozmoWin).wait_for_completed()  # pyright: ignore[reportAttributeAccessIssue]
        else:
            await self.robot.play_anim_trigger(cozmo.anim.Triggers.OnSpeedtapHandPlayerWin).wait_for_completed()  # pyright: ignore[reportAttributeAccessIssue]
    ...
    async def assign_cubes(self):
        ...
        self.player.cube.stop_light_chaser()  # pyright: ignore[reportOptionalMemberAccess]
        self.assign_countdown_cube()

    def blink_available_cubes(self):
        '''Blinks the cubes which Cozmo did not select for himself.'''
        for cube in self.cubes:  # pyright: ignore[reportOptionalIterable]
            if cube.object_id != self.cozmo_player.cube.object_id:  # pyright: ignore[reportOptionalMemberAccess]
                cube.start_light_chaser(0.5)  # pyright: ignore[reportOptionalMemberAccess]
    ...
    def assign_countdown_cube(self):
        ...
        for cube in self.cubes:  # pyright: ignore[reportOptionalIterable]
            if cube.object_id != self.cozmo_player.cube.object_id and cube.object_id != self.player.cube.object_id:  # pyright: ignore[reportOptionalMemberAccess]
                self.countdown_cube = cube
                self.countdown_cube.stop_light_chaser()  # pyright: ignore[reportOptionalMemberAccess]
    ...
    def turn_on_buzzer_cubes_same(self):
        ...
        self.player.cube.set_light_corners(*same_colors)  # pyright: ignore[reportOptionalMemberAccess]
        self.cozmo_player.cube.set_light_corners(*same_colors)  # pyright: ignore[reportOptionalMemberAccess]
    ...
    def turn_on_buzzer_cubes_different(self):
        ...
        while player_cube_colors == cozmo_cube_colors:
            cozmo_cube_colors = self.generate_random_buzzer_colors()
        self.player.cube.set_light_corners(*player_cube_colors)  # pyright: ignore[reportOptionalMemberAccess]
        self.cozmo_player.cube.set_light_corners(*cozmo_cube_colors)  # pyright: ignore[reportOptionalMemberAccess]

    def turn_on_buzzer_cubes_red(self):
        '''Sets the buzzer cubes to red.'''
        self.player.cube.set_lights(cozmo.lights.red_light)  # pyright: ignore[reportOptionalMemberAccess]
        self.cozmo_player.cube.set_lights(cozmo.lights.red_light)  # pyright: ignore[reportOptionalMemberAccess]
    ...
    def turn_off_buzzer_cubes(self):
        ...
        self.player.cube.set_lights_off()  # pyright: ignore[reportOptionalMemberAccess]
        self.cozmo_player.cube.set_lights_off()  # pyright: ignore[reportOptionalMemberAccess]
    ...
    def cubes_connected(self):
        ...
        self.cubes = [cube1, cube2, cube3]  # pyright: ignore[reportAttributeAccessIssue]
    ...
    async def report_winner(self):
        ...
        if self.cozmo_player.score > self.player.score:
            ...
            await self.robot.play_anim_trigger(cozmo.anim.Triggers.OnSpeedtapGameCozmoWinHighIntensity).wait_for_completed()  # pyright: ignore[reportAttributeAccessIssue]
        else:
            print('~PLAYER WINS QUICK TAP~')
            await self.robot.play_anim_trigger(cozmo.anim.Triggers.OnSpeedtapGamePlayerWinHighIntensity).wait_for_completed()  # pyright: ignore[reportAttributeAccessIssue]


class QuickTapPlayer():
    '''Player-specifc Quick Tap logic.'''
    def __init__(self):
        self.cube: Optional[BlinkyCube] = None
        self.score = 0
        self.has_tapped = False
        self.name = 'Player'
        self.elapsed_tap_time: Optional[float] = None
...
class CozmoQuickTapPlayer(QuickTapPlayer):
    ...
    async def select_cube(self):
        ...
        self.cube.set_lights(cozmo.lights.white_light)  # pyright: ignore[reportOptionalMemberAccess]
        await asyncio.sleep(2)
        self.cube.start_light_chaser(0.5)  # pyright: ignore[reportOptionalMemberAccess]
        await self.robot.set_lift_height(1.0).wait_for_completed()
        await self.robot.go_to_object(self.cube, distance_mm(40)).wait_for_completed()
        await self.robot.play_anim_trigger(cozmo.anim.Triggers.OnSpeedtapTap).wait_for_completed()  # pyright: ignore[reportAttributeAccessIssue]
        self.cube.stop_light_chaser()  # pyright: ignore[reportOptionalMemberAccess]
        self.cube.set_lights(green_light)  # pyright: ignore[reportOptionalMemberAccess]
    ...
    async def fail_to_tap(self):
        ...
        if probability_fakeout < 0.5:
            await self.robot.play_anim_trigger(cozmo.anim.Triggers.OnSpeedtapFakeout).wait_for_completed()  # pyright: ignore[reportAttributeAccessIssue]
        else:
            await self.robot.play_anim_trigger(cozmo.anim.Triggers.OnSpeedtapIdle).wait_for_completed()  # pyright: ignore[reportAttributeAccessIssue]
