...
class ColorFinder(cozmo.annotate.Annotator):
    ...
    def __init__(self, robot: cozmo.robot.Robot):
        ...
        self.color_selector_cube: Optional[cozmo.objects.LightCube] = None
        self.color_to_find = 'yellow'
        self.color_to_find_index = POSSIBLE_COLORS_TO_FIND.index(self.color_to_find)

        self.grid_cube: Optional[cozmo.objects.LightCube] = None
        ...
        self.look_around_behavior: Optional[cozmo.behavior.Behavior] = None
        self.drive_action: Optional[cozmo.robot.DriveStraight] = None
        self.tilt_head_action: Optional[cozmo.robot.SetHeadAngle] = None
        self.rotate_action: Optional[cozmo.robot.TurnInPlace] = None
        self.lift_action: Optional[cozmo.robot.SetLiftHeight] = None

        self.last_known_blob_center = (DOWNSIZE_WIDTH/2, DOWNSIZE_HEIGHT/2) # initially set to center screen

        self.white_balance_cube: Optional[cozmo.objects.LightCube] = None
    ...
    def on_cube_tap(self, evt, obj, **kwargs):
        ...
        '''    
        if obj.object_id == self.color_selector_cube.object_id:  # pyright: ignore[reportOptionalMemberAccess]
            self.toggle_color_to_find()
        elif obj.object_id == self.grid_cube.object_id:  # pyright: ignore[reportOptionalMemberAccess]
            self.robot.world.image_annotator.annotation_enabled = not self.robot.world.image_annotator.annotation_enabled
        elif obj.object_id == self.white_balance_cube.object_id:  # pyright: ignore[reportOptionalMemberAccess]
            self.white_balance()
    ...
    def toggle_color_to_find(self):
        ...
        self.color_selector_cube.set_lights(map_color_to_light[self.color_to_find])  # pyright: ignore[reportOptionalMemberAccess]
    ...
    def get_low_res_view(self):
        ...
        '''
        image = self.robot.world.latest_image.raw_image
        downsized_image = image.resize((DOWNSIZE_WIDTH, DOWNSIZE_HEIGHT), resample = Image.Resampling.LANCZOS)
    ...
    def turn_on_cubes(self):
        ...
        self.color_selector_cube.set_lights(map_color_to_light[self.color_to_find])  # pyright: ignore[reportOptionalMemberAccess]
        self.grid_cube.set_lights(cozmo.lights.white_light.flash())  # pyright: ignore[reportOptionalMemberAccess]
