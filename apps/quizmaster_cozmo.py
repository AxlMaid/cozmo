...
class CozmoQuizPlayer:
    ...
    def __init__(self,
                 ...
                 cube: Optional[cozmo.objects.LightCube], index, color, name):
        self._robot = robot
        self._cube = cube
        self._index = index
        self._color = color
...
class CozmoQuizMaster:
    ...
    def __init__(self, robot: cozmo.robot.Robot):
        ...
        self._players: List[CozmoQuizPlayer] = []
        ...
        self._answering_player: Optional[CozmoQuizPlayer] = None
    ...
    async def get_correct_player(self, question: QuizQuestion):
        ...
        for _ in range(len(self._players)):
            ...
            if player_answer == question.answer_number:
                ...
                # Incorrect
                player.score -= 1
                action = self._robot.play_anim_trigger(cozmo.anim.Triggers.KnockOverFailure)  # pyright: ignore[reportAttributeAccessIssue]
    ...
    async def ask_question(self, question: QuizQuestion):
        ...
        if correct_player is None:
            ...
            action = self._robot.play_anim_trigger(cozmo.anim.Triggers.FailedToRightFromFace)  # pyright: ignore[reportAttributeAccessIssue]
            await action.wait_for_completed()
            action = self.say_text("The answer was %s: %s" % (question.answer_number, question.answer_str))
            await action.wait_for_completed()
        else:
            # Correct
            action = self._robot.play_anim_trigger(cozmo.anim.Triggers.ReactToBlockPickupSuccess)  # pyright: ignore[reportAttributeAccessIssue]
            await action.wait_for_completed()
    ...
    async def run(self):
        ...
        while True:
            ...
            if question:
                ...
                action = self._robot.play_anim_trigger(cozmo.anim.Triggers.BuildPyramidSuccess)  # pyright: ignore[reportAttributeAccessIssue]
                await action.wait_for_completed()
