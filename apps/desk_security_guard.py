...
class DeskSecurityGuard:
    ...
    def __init__(self):
        ...
        self.time_first_observed_intruder: Optional[float] = None
        self.time_last_observed_intruder: Optional[float] = None

        self.time_first_observed_owner: Optional[float] = None
        self.time_last_observed_owner: Optional[float] = None

        self.time_last_suspicious: Optional[float] = None
        self.time_last_uploaded_photo: Optional[float] = None
        self.time_last_announced_intruder: Optional[float] = None
        self.time_last_pounced_at_intruder: Optional[float] = None
        self.time_last_announced_owner: Optional[float] = None
...
async def check_for_intruder(robot, dsg:DeskSecurityGuard):
    ...
    if can_see_owner:
        ...
        if not did_occur_recently(dsg.time_last_announced_owner, 60.0):
            await robot.play_anim_trigger(cozmo.anim.Triggers.NamedFaceInitialGreeting).wait_for_completed()  # pyright: ignore[reportAttributeAccessIssue]
        ...
        if is_confirmed_intruder:
            ...
            if not did_occur_recently(dsg.time_last_pounced_at_intruder, 10.0):
                await robot.play_anim_trigger(cozmo.anim.Triggers.CubePouncePounceNormal).wait_for_completed()  # pyright: ignore[reportAttributeAccessIssue]
            ...
            if not did_occur_recently(dsg.time_last_suspicious, 10.0):
                await robot.play_anim_trigger(cozmo.anim.Triggers.HikingInterestingEdgeThought).wait_for_completed()  # pyright: ignore[reportAttributeAccessIssue]
